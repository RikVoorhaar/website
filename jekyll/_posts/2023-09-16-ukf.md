...
take lots of samples $(x_1,\ldots,x_N)$ of $x$, and then compute the mean and covariance
of $(f(x_1),\ldots,f(x_N))$. The only issue with this is that we need many samples in
order to get an accurate estimate. The trick is instead to use _deterministic sampling_ and _choose_ a good set of points $(s_1,\dots,s_N)$ and then estimate the mean using a weighted mean $\mu=\sum_{i=1}^Nf(\sigma_i)w^a_i$. The covariance is then estimated using the estimated mean and a second set of weights: $\Sigma = \sum_i w_i^c (f(s_i)-\mu)(f(s_i)-\mu)^\top$. 

> Explain van der Merwe sigma pont algorihm
